...
void process_diff(const bk_tree<wstring>& bk, vector<info_word>& v, wstring s, long long web) {
	info_word info;
	info.word = s;
...
void process_easy(const bk_tree<wstring>& bk, vector<info_word>& v, wstring s, long long web) {
	info_word info;
	info.word = s;
	info.vowels = num_vowels(s);
	info.has_accent = has_accent(s);
...
void init_classes_list() {
	...
	v10.push_back(L"xtr");
	v10.push_back(L"nsp");
	classes_list.push_back(v10);
	vector<wstring> v11;
	v11.push_back(L"gu");
	classes_list.push_back(v11);	
	vector<wstring> v12;
	v12.push_back(L"rr");
	v12.push_back(L"ll");
...
void print_arff(const vector<info_word>& v) {
	...
	wcout << "@attribute d2 integer" << L"\n";
	wcout << "@attribute vowels integer" << L"\n";
	wcout << "@attribute c1 {yes,no}" << L"\n";
	wcout << "@attribute c2 {yes,no}" << L"\n";
	wcout << "@attribute c3 {yes,no}" << L"\n";
	wcout << "@attribute c4 {yes,no}" << L"\n";
	wcout << "@attribute c5 {yes,no}" << L"\n";
	wcout << "@attribute c6 {yes,no}" << L"\n";
	wcout << "@attribute c7 {yes,no}" << L"\n";
	wcout << "@attribute c8 {yes,no}" << L"\n";
	wcout << "@attribute c9 {yes,no}" << L"\n";
	wcout << "@attribute c10 {yes,no}" << L"\n";
	wcout << "@attribute c11 {yes,no}" << L"\n";
	wcout << "@attribute c12 {yes,no}" << L"\n";
	wcout << "@attribute w_length integer" << L"\n";
	wcout << "@attribute v_percentage real" << L"\n";
	wcout << "@attribute three_consonants {yes,no}" << L"\n";
	wcout << "@attribute two_vowels {yes,no}" << L"\n";
	wcout << "@attribute has_x {yes,no}" << L"\n";
	wcout << "@attribute alternada {yes,no}" << L"\n";
	wcout << "@attribute has_h {yes,no}" << L"\n";
	wcout << "@attribute has_double_letter {yes,no}" << L"\n";
	wcout << "@attribute webhits integer" << L"\n";
	...
	for (int i=0; i<v.size(); i++) {
		...
		//if (info.has_accent)	wcout << L", " << L"yes";
		//else wcout << L", " << L"no";
		wcout << L", " << info.classes[0];
		wcout << L", " << info.classes[1];
		wcout << L", " << info.classes[2];
		wcout << L", " << info.classes[3];
		wcout << L", " << info.classes[4];
		wcout << L", " << info.classes[5];
		wcout << L", " << info.classes[6];
		wcout << L", " << info.classes[7];
		wcout << L", " << info.classes[8];
		wcout << L", " << info.classes[9];
		wcout << L", " << info.classes[10];
		wcout << L", " << info.classes[11];
		
		wcout << L", " << info.length;

		wcout << L", " << info.percentage_vowels;

		if (info.three_consonants) wcout << L", " << L"1";
		else wcout << L", " << L"0";

		if (info.two_vowels) wcout << L", " << L"1";
		else wcout << L", " << L"0";

		if (info.has_x) wcout << L", " << L"1";
		else wcout << L", " << L"0";

		if (info.alternada) wcout << L", " << L"1";
		else wcout << L", " << L"0";

		if (info.has_h) wcout << L", " << L"1";
		else wcout << L", " << L"0";

		if (info.has_double_letter) wcout << L", " << L"1";
		else wcout << L", " << L"0";

		wcout << L", " << info.web;

		if (info.has_o_and_u) wcout << L", " << L"1";
		else wcout << L", " << L"0";

		if (info.has_ua) wcout << L", " << L"1";
		else wcout << L", " << L"0";
...
int main()
{
	...
	std::fstream finw("../learning_words/sets/difficult_words_unique_webhits.txt", std::fstream::in);
	...
	std::fstream fin2w("../learning_words/sets/easy_words_webhits.txt", std::fstream::in);
	
	//process the easy words
	for(wstring s; getline(fin2,s); ) {
		long long web;
		fin2w >> web;
		process_easy(bk,v,s,web);
	}
