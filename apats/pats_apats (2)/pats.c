...
{	
	...
	fprintf(stderr,"Process Time: %.4lf\n",elapsed);
...
{	
	...
	readPatterns(argv[1]);
	fprintf(stderr,"Patterns read\n");
	preProcess();
	fprintf(stderr,"pre Processed\n");
	clock_t fi = clock();
	double elaps = ((double) (fi - ini)) / CLOCKS_PER_SEC;	
	fprintf(stderr,"Building Time: %.4lf\n",elaps);
